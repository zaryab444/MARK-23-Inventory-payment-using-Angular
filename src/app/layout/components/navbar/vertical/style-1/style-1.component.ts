...
export class NavbarVerticalStyle1Component implements OnInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        if(this.themeSettings && this.themeSettings.splashScreenImageUrl !=  null) {
    ...
    checkForEnvironment(themeSettings) {
        if(environment.dev == true) {
            this.logo = `${environment.apiURL.split('api/').shift()}${'api/'}${themeSettings.splashScreenImageUrl.split("~/").pop()}`
        } else {
            this.logo = `${environment.apiURL.split('api/').shift()}${themeSettings.splashScreenImageUrl.split("~/").pop()}`
        }
    }
    /**
     * On destroy
